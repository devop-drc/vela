...
const ResetPassword = () => {
  ...
  const onSubmit = async (data: ResetFormData) => {
    ...
    if (error) {
      showError(toFriendlyError(error, tr("misc.reset_pw_failed", "Couldn't update your password. Please request a new reset link.")));
      return;
    }
    setDone(true);
    showSuccess(tr("misc.reset_pw_success", "Password updated. You can now sign in."));
      ...
      <div ref={contentRef}>
        {branch === 'done' ? (
          <div className="flex flex-col items-center gap-3 py-6 text-center" data-reveal>
            ...
            <p className="text-muted-foreground">{tr("misc.reset_pw_done", "Your password has been updated. Redirecting you to sign in…")}</p>
          </div>
        ) : branch === 'invalid' ? (
          <div className="space-y-4 py-6 text-center" data-reveal>
            <p className="text-muted-foreground">
              {tr("misc.reset_link_invalid", "This password reset link is invalid or has expired. Please request a new one from the login page.")}
            </p>
            <Button asChild variant="outline"><Link to="/login">{tr("misc.back_to_login", "Back to login")}</Link></Button>
          </div>
        ) : branch === 'loading' ? (
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
            <div className="space-y-2" data-reveal>
              <Label htmlFor="password">{tr("misc.new_password", "New password")}</Label>
              <div className="relative">
                <Lock className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
                <Input id="password" type={showPassword ? "text" : "password"} autoComplete="new-password" placeholder={tr("misc.at_least_8", "At least 8 characters")} {...register('password')} className="h-11 rounded-xl pl-10 pr-10" />
                <button type="button" onClick={() => setShowPassword((v) => !v)}
                  aria-label={showPassword ? tr("misc.hide_password", "Hide password") : tr("misc.show_password", "Show password")}
                  className="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground transition-colors hover:text-foreground">
                  {showPassword ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}
                </button>
              </div>
              {errors.password && <p className="text-sm text-destructive">{tr("misc.password_min", "Password must be at least 8 characters")}</p>}
            </div>
            <div className="space-y-2" data-reveal>
              <Label htmlFor="confirm">{tr("misc.confirm_new_password", "Confirm new password")}</Label>
              <div className="relative">
                <Lock className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
                ...
                <button type="button" onClick={() => setShowConfirm((v) => !v)}
                  aria-label={showConfirm ? tr("misc.hide_password", "Hide password") : tr("misc.show_password", "Show password")}
                  className="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground transition-colors hover:text-foreground">
                  {showConfirm ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}
                </button>
              </div>
              {errors.confirm && <p className="text-sm text-destructive">{tr("misc.passwords_no_match", "Passwords don't match")}</p>}
            </div>
            <Button type="submit" disabled={isSubmitting} data-reveal
              ...
              {tr("misc.update_password", "Update password")}
            </Button>
            <p className="text-center text-sm text-muted-foreground" data-reveal>
              <Link to="/login" className="font-medium text-primary hover:underline">{tr("misc.back_to_login", "Back to login")}</Link>
            </p>
          </form>
        )}
      </div>
