...
export const CartContents = ({ onClose, variant = 'drawer' }: { onClose?: () => void; variant?: 'drawer' | 'page' }) => {
  ...
  if (cartItems.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center flex-1 p-10 text-center min-h-[40vh]">
        ...
        <h3 className="sf-heading text-2xl font-bold mb-2">{t('emptyCart')}</h3>
        <p className="text-muted-foreground mb-6">{t('browseShop')}</p>
        <SfButton onClick={onClose}>{t('startShopping')}</SfButton>
      </div>
    );
  }
  ...
  const freeShipHint = step === 'cart' && shipping > 0 && freeShippingThreshold > subtotal ? (
    <div className="space-y-1.5 rounded-md bg-muted/50 px-3 py-2.5">
      <div className="flex items-center gap-2 text-xs text-muted-foreground">
        ...
        <span>{t('freeShipPrefix')} <span className="font-semibold text-foreground">{formatCurrency(freeShippingThreshold - subtotal, shopDetails?.currency)}</span> {t('freeShipSuffix')}</span>
      </div>
  ...
  const totals = (
    <div className="space-y-3">
      <div className="flex justify-between text-sm"><span>{t('subtotal')} ({totalItems})</span><span className="font-semibold">{formatCurrency(subtotal, shopDetails?.currency)}</span></div>
      <div className="flex justify-between text-sm"><span>{t('shipping')}</span><span className={cn('font-semibold', shipping === 0 && 'text-success')}>{shipping === 0 ? t('free') : formatCurrency(shipping, shopDetails?.currency)}</span></div>
      {totalSaved > 0 && <div className="flex justify-between text-sm text-success"><span>{t('youSaved')}</span><span className="font-semibold">{formatCurrency(totalSaved, shopDetails?.currency)}</span></div>}
      <Separator />
      <div className="flex justify-between text-lg font-bold"><span>{t('total')}</span><span>{formatCurrency(total, shopDetails?.currency)}</span></div>
    </div>
  );

  const cta = (
    <>
      {step === 'cart' && <SfButton className="w-full" onClick={() => setStep('contact-shipping')}>{t('proceedToCheckout')} <ArrowRight className="ml-2 h-4 w-4" /></SfButton>}
      {step === 'contact-shipping' && <SfButton type="submit" form="sf-checkout-form" className="w-full">{t('continueToPayment')} <ArrowRight className="ml-2 h-4 w-4" /></SfButton>}
      {step === 'payment' && (
        <SfButton type="submit" form="sf-checkout-form" className="w-full" disabled={isSubmitting}>
          {isSubmitting ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" /> {t('placingOrder')}</> : <><CheckCircle className="mr-2 h-4 w-4" /> {t('placeOrder')}</>}
        </SfButton>
      )}
    </>
  );
  ...
  const subscriptionNote = hasSubscriptionProducts ? (
    ...
    >
      <Info className="h-4 w-4 shrink-0" style={{ color: 'hsl(var(--warning))' }} /> {t('subscriptionNote')}
    </div>
  ) : null;
      ...
      <div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_380px] lg:items-start">
        <div className="min-w-0 space-y-5">
          <div className="flex items-center gap-2">
            ...
            <h1 className="sf-heading text-2xl font-bold flex items-center gap-2"><ShoppingBag className="h-6 w-6" /> {step === 'cart' ? t('yourCart') : t('checkout')}</h1>
          </div>
          {step === 'cart' ? (
        ...
        <aside className="lg:sticky lg:top-24">
          <div className="sf-glass p-5 space-y-4">
            <h2 className="sf-heading text-lg font-bold">{t('orderSummary')}</h2>
            ...
            {totals}
            {cta}
            <div className="flex items-center justify-center gap-1.5 text-xs text-muted-foreground"><ShieldCheck className="h-3.5 w-3.5" /> {t('secureCheckout')}</div>
          </div>
        </aside>
      </div>
    );
  ...
  return (
    <div className="flex flex-col h-full min-h-0">
      <div className="flex items-center gap-2 p-4 border-b shrink-0">
        {step !== 'cart' && <Button variant="ghost" size="icon" className="h-8 w-8" onClick={back}><ArrowLeft className="h-4 w-4" /></Button>}
        <h2 className="sf-heading text-xl font-bold flex items-center gap-2"><ShoppingBag className="h-5 w-5" /> {step === 'cart' ? t('yourCart') : t('checkout')}</h2>
      </div>
