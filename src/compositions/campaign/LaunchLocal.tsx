...
import { BRAND, CLASH, INTER, AuroraDark, Shimmer, ShipWhite, glassDark, ensureClash } from "../marketing/nextgen/kitv2";
      ...
      <AbsoluteFill style={{ padding: "92px 84px 86px", zIndex: 2, display: "flex", flexDirection: "column" }}>
        ...
        <div style={{ flex: 1, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24, margin: "44px 0" }}>
          {FEATURES.map(({ Icon, title, text }) => (
            <div key={title} style={{ ...glassDark, borderRadius: 28, padding: "40px 36px", display: "flex", flexDirection: "column", position: "relative", overflow: "hidden" }}>
              <span style={{ position: "absolute", inset: 0, pointerEvents: "none", background: "radial-gradient(130% 80% at 18% -12%, rgba(255,255,255,0.14), transparent 52%)" }} />
              <span style={{ position: "absolute", top: 0, left: 34, right: 34, height: 2, borderRadius: 9, background: "linear-gradient(90deg,transparent,#FF2E4D,#FACC15,transparent)", opacity: 0.7 }} />
              <span style={{ width: 76, height: 76, borderRadius: 20, display: "grid", placeItems: "center", background: "rgba(250,204,21,0.16)", border: "1px solid rgba(250,204,21,0.25)", marginBottom: 24, zIndex: 1 }}><Icon size={40} color={BRAND.yellow} strokeWidth={1.8} /></span>
              <div style={{ fontFamily: CLASH, fontWeight: 600, fontSize: 34, color: "#fff", marginBottom: 12, zIndex: 1 }}>{title}</div>
              <div style={{ fontFamily: INTER, fontSize: 25, lineHeight: 1.45, color: "rgba(255,255,255,0.66)", zIndex: 1, position: "relative" }}>{text}</div>
            </div>
          ))}
        </div>
