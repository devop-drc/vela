...
export const TemplateMarquee = ({ activeTemplateId, onPick, className }: Props) => {
  ...
  return (
    <div className={cn('flex min-h-0 flex-col', className)}>
      <p className="shrink-0 px-3 pb-2 pt-3 text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">Templates</p>
      <div
        ref={railRef}
          ...
          {doubled.map((t, i) => {
            ...
            return (
              <button
                key={`${t.id}-${i}`}
                type="button"
                onClick={() => { if (!drag.current?.moved) onPick(t.config, t.id); }}
                className={cn(
              ...
              >
                <ScaledFrame src={`/demo-shop?template=${t.id}`} virtualW={1280} virtualH={760} title={`${t.name} template`} className="pointer-events-none rounded-none border-0" />
                {active && (
      ...
      <p className="shrink-0 px-3 pb-2 text-center text-[10px] text-muted-foreground">Drag to browse · click to apply</p>
    </div>
  );
};
