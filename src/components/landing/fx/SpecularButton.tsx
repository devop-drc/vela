...
export interface SpecularButtonProps {
  ...
  className?: string;
  /** Vela adaptation: class for the fill layer (e.g. "brand-gradient"); overrides the flat tint. */
...
const SpecularButton = ({
  ...
  return (
    <button
      ...
      disabled={disabled}
      onClick={onClick}
      className={`relative m-0 inline-flex cursor-pointer items-center justify-center border-none font-medium leading-none tracking-[0.01em] outline-none transition-transform duration-150 active:scale-[0.97] disabled:cursor-default disabled:opacity-55 disabled:active:scale-100 [color:var(--sb-text-color)] [border-radius:var(--sb-radius)] shadow-[inset_0_1px_0_rgba(255,255,255,0.04),0_8px_24px_rgba(0,0,0,0.25)] focus-visible:outline-2 focus-visible:outline-offset-[3px] ${SIZES[size] || SIZES.md}${className ? ` ${className}` : ''}`}
      style={
        {
          '--sb-radius': `${radius}px`,
          '--sb-tint': tint,
    ...
    >
      {/* Vela adaptation: the fill is its own layer so callers can hand it a
          gradient (backgroundClassName="brand-gradient") instead of the flat tint. */}
      <span
        aria-hidden="true"
        className={`pointer-events-none absolute inset-0 z-0 [border-radius:var(--sb-radius)] [backdrop-filter:blur(var(--sb-blur))] ${
          backgroundClassName ?? '[background:color-mix(in_srgb,var(--sb-tint)_calc(var(--sb-tint-opacity)*100%),transparent)]'
        }`}
      />
      ...
      <span className="relative z-[2] inline-flex items-center gap-2 whitespace-nowrap">{children}</span>
    </button>
  );
};
