...
const Frame = ({ src, url }: { src: string; url: string }) => (
  <div className="overflow-hidden rounded-2xl border border-border bg-card shadow-[0_40px_90px_-30px_rgba(30,10,50,0.32)]">
...
/**
 * S6 — how it works. The two-column block PINS dead-centre in the viewport;
 * scrolling advances the active step and crossfades the visual. GSAP
 * ScrollTrigger pin handles spacing exactly → no header overlap, no dead space.
 */
    ...
    const ctx = gsap.context(() => {
      ...
      const setActive = (n: number) => {
        ...
        gsap.utils.toArray<HTMLElement>(".hiw-copy").forEach((el, i) =>
          gsap.to(el, { opacity: i === n ? 1 : 0.34, duration: 0.4, overwrite: "auto" }),
        );
        gsap.utils.toArray<HTMLElement>(".hiw-badge").forEach((el, i) =>
          gsap.to(el, { scale: i === n ? 1 : 0.9, duration: 0.4, ease: "power2.out", overwrite: "auto" }),
        );
      };
              ...
              {steps.map((s, i) => (
                <div key={i} className="hiw-step relative pl-16">
                  ...
                  <div className="hiw-copy">
                    <div className="text-[12px] font-bold uppercase tracking-[0.18em] text-muted-foreground">{t(lang, "Hapi", "Step")} {i + 1}</div>
                    <h3 className="mt-1 font-display-brand text-[24px] font-bold tracking-tight text-foreground">{s.title}</h3>
