      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {attributes.map((attr) => (
            <div key={attr.name} className="space-y-2">
                ...
                {renderField(control, attr.name, attr.inputType)}
            </div>
        ))}
        {fields.map((field: any, index) => (
            <div key={field.id} className="space-y-2">
                ...
                <Controller name={`${isOptions ? "dynamicOptions" : "dynamicSpecs"}.${index}.value`} control={control} render={({ field: rhfField }) => <Input {...rhfField} />} />
            </div>
        ))}
      </div>
      {isAdding ? (
        <div className="flex items-end gap-2 p-2 border rounded-md">
            ...
            <div className="space-y-1"><Label>Field Type</Label><Select value={newFieldType} onValueChange={setNewFieldType}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="text">Text</SelectItem><SelectItem value="number">Number</SelectItem><SelectItem value="color">Color</SelectItem><SelectItem value="tags">Tags</SelectItem></SelectContent></Select></div>
