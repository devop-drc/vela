  ...
  ({ product, shopDetails, convertCurrency, promotions }, ref) => {
    ...
    const actuallyBuyNow = () => {
      ...
      const itemToBuy: CartItem = {
        uid: `${product.id}::${variantKey || 'base'}`,
        productId: product.id,
        name: product.name,
        price: hasDiscount ? discountedPrice : originalDisplayPrice,
        originalPrice: originalDisplayPrice,
    ...
    return (
      <div ref={ref} id={`product-${product.id}`} className="bg-[hsl(var(--card))] text-[hsl(var(--foreground))] border-b pb-4">
        ...
        <div className="flex items-center gap-3 p-4">
          ...
          <div>
            <p className="font-semibold text-sm leading-tight">{product.name}</p> {/* Product Name */}
            ...
            <CarouselContent>
              {mediaItems.map((url: string, index: number) => (
                <CarouselItem key={index}>
                  <div className="relative aspect-square w-full flex items-center justify-center" style={{backgroundColor: 'hsl(var(--muted))'}}>
        ...
        <div className="p-4 space-y-3">
          ...
          <div className="flex items-center gap-3">
            {hasDiscount && originalDisplayPrice !== null ? (
              ...
              <p className="text-2xl font-bold text-[hsl(var(--foreground))]">
                ...
                {product.pricing_type === 'subscription' && (
                  <span className="text-base font-light text-[hsl(var(--muted-foreground))]">/{product.billing_interval === 'month' ? 'mo' : 'yr'}</span>
                )}
              </p>
            )}
          </div>
          ...
          {product.tags && product.tags.length > 0 && (
            <div className="flex flex-wrap gap-2">
              {product.tags.map((tag: string) => <Badge key={tag} variant="outline" className="text-xs border" style={{backgroundColor: 'hsl(var(--muted))', color: 'hsl(var(--foreground))', borderColor: 'hsl(var(--border))'}}>{tag}</Badge>)}
            </div>
          )}
            ...
            <div className="flex items-stretch gap-3 pt-3"> {/* Changed to items-stretch for vertical alignment */}
              ...
              <div className="flex flex-col border rounded-md overflow-hidden flex-shrink-0 w-16" style={{borderColor:'hsl(var(--border))'}}> {/* Fixed width for counter */}
                <Button
                  ...
                  className="h-full w-full text-[hsl(var(--foreground))] hover:bg-[hsl(var(--muted))]"
                >
                ...
                <Input
                  ...
                  className="w-full text-center border-y border-x-0 focus-visible:ring-0 text-base h-full p-0 bg-[hsl(var(--card))] text-[hsl(var(--foreground))]"
