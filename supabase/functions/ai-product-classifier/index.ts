...
serve(async (req) => {
  ...
  try {
    ...
    if (!userId) throw new Error("User ID is required for classification.");
    ...
    if (categoryName && typeName) {
      const categoryId = await upsertCategory(supabaseAdmin, categoryName, userId);
      await upsertTypeAndMergeAttributes(supabaseAdmin, categoryId, typeName, attributes || [], userId);
    }
