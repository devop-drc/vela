...
serve(async (req) => {
  ...
  try {
    ...
    // 8. Stock counts changed — drop cached public storefront payloads so
    // shoppers see fresh availability. Fails open inside the helper.
    await invalidateShopCache({ slug: shopSlug, businessId });
